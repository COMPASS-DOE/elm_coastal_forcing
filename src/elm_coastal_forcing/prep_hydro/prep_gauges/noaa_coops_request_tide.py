...
def query_noaa_hourly_tide_gauge(station_id, start_date, datum, output_dir):
    ...
    params = {
        ...
        "product": "hourly_height",    # Request water level data
        "datum": datum,                # "IGLD" or  "NAVD"  # Vertical datum (use MLLW for mean lower low water)
...
def save_noaa_coops_wse(gauge_df, years, outdir):
    ...
    for index, row in gauge_df.iterrows():
        ...
        # outname = f'{outdir}/swe_noaa_coops_{site_id}_{station_id}.csv'
        outname = f'{outdir}/swe_noaa_coops_{station_id}.csv'
...
if __name__ == '__main__':
    ...
    # output dir
    outdir = str(DATA_DIR/'tide_gauges/noaa_coops/swe')

    #%% Run function
    save_noaa_coops_wse(noaa_gauges, years, outdir)

    # Read in gauge_df
    # gauge_df = (
    #     pd.read_csv('../../data/tide_gauges/all_gauges_list/synoptic_tide_gauges.csv', dtype=str)
    #     .query('data_source == "NOAA"'))
