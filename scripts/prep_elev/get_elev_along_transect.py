...
#%% Reread & Clean up the output file


allsites_elev = (pd.read_csv('../../output/results/elev/old/all_sites_elev_profile.csv', index_col=False)
                 .reset_index(drop=True)
                                  .rename(columns={'dist': 'dist_from_upland'})
                #  .rename(columns={'dist_from_upland': 'dist'})
                .rename(columns={'elev_fromwetland':'elev_above_wetland'})
                )

# Reorder columns
allsites_elev = allsites_elev[['site_type', 'region','site_id', 'zone', 'dist_from_upland', 'elev', 'elev_above_wetland']]\

# Save to CSV file
allsites_elev.to_csv('../../output/results/elev/old/all_sites_elev_transect_v01.csv', index=False)
