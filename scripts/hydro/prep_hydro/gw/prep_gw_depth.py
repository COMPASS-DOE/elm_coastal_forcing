...
# rsync -avz <user>@compass.pnl.gov:/compass/datasets/fme_data_release/sensor_data/Level1/v1-2
...
#%% --------------------------------------------------------------------
#/  Function to extract and filter CSV files from zip archives
#   Applied to the synoptic sensor data releases
...
#%%--------------------------------------------------------------------
#/  Apply the data prep function to the synoptic data directory

# Set directory where sensor data is stored
directory = '../../data/synoptic_sensors/L1_v12'


# Combine all gw_pressure sensor data together
gw_depth_df = extract_and_filter_zip_files(directory)

# Pivot table to columns
gw_depth_df = (
    gw_depth_df
    .drop(columns=['ID'])
    .reset_index()
    # Remove these two NaN filled columns: 'Location','Sensor_ID',
...
# gw_depth_df_copy = gw_depth_df.copy()

gw_depth_df = gw_depth_df_copy.copy()
...
#%%--------------------------------------------------------------------
# Join well depth measure with DEM elev to convert to height from NADV88

# Read in table with full names
# TODO: Fix the elevation order in Gcrew and other sites

synoptic_elev = (
    pd.read_csv('../../COMPASS_synoptic_sims/data/processed/synoptic_site_pts/synoptic_elev_zone_v4.csv')
    .drop(['dem_tile','nwi_file'], axis=1)
)
...
well_dimensions = pd.read_csv('../../data/synoptic_sensors/L1_v12/Aquatroll_installation_info_042025.csv').iloc[:, 0:14]
# well_dimensions = pd.read_csv('../../data/synoptic_sensors/L1_v11/Aquatroll_installation_info_092024.csv').iloc[:, 0:14]
...
#%% Join well dimension to gw_depth_df
gw_depth_df = (
...
# Compute water elevation based on pressure, density, and well dimensions
gw_depth_df = (
    gw_depth_df
    .assign(
...
gw_depth_df = gw_depth_df[['region_id','region_name',  'site_id', 'site_name', 'zone_name', 'zone_id','zone_num',
                           'lat', 'long', 'dist', 'elev_fromwetland', 'elev_ft', 'elev_m', 
                            'TIMESTAMP_hourly', 'Instrument', 'gw_density', 'gw_pressure', 'gw_salinity', 'gw_temperature', 
                            # 'install_date', 'logger_id', 'logger_name', 'serial_number',
                            # 'gps_notes', 
                            'well_top_to_ground_cm', 'ring_to_pressure_sensor_cm', 'bolt_to_cap_cm', 'elevation_masl',
                            'ground_to_sensor_cm', 'pressurehead_m', 'wl_below_surface_m','gw_elev_m']]
                            # 'salinity_rolling_mean', 'salinity_rolling_std',
                            # 'salinity_z_score', 'elev_rolling_mean', 'elev_rolling_std',
                            # 'elev_z_score'


# /--------------------------------------------------------------------
#/  Write synoptic well data to a CSV file
output_filename = '../../output/results/sensor_gauges/synoptic_gw_elev_v4.csv'
