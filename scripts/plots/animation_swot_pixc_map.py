...
# s.set_norm(norm)
# s.set_norm(norm)
# norm = mcolors.Normalize(vmin=-3, vmax=8, clip=False)
# cbar = fig.colorbar(mappable=scatter, ax=ax)

#cmap=plt.cm.plasma) # nipy_spectral
# s = ax.scatter(vmin=-5, vmax=20) 
# Scatter plot
# fig, ax = plt.subplots()
# scatter = ax.scatter(x, y, c=values, cmap=cmap, norm=norm, edgecolor='black')

# transect = gpd.read_file('../../output/results/transect_line/transect_line' + site_name + '.shp').to_crs('EPSG:4326')
# synoptic_pts = synoptic_pts.to_crs(swot_crs)
# site_bbox = gpd.GeoDataFrame([site_bbox], geometry='geometry', crs='EPSG:4326')

# plt.text(transect_pts.geometry.x, transect_pts.geometry.y, s=transect_pts['zone_id'].tolist())
	# fontsize=16, 
	# ha='right', va='bottom')

# print(f"GIF created and saved as {gif_file}")

# import imageio.v3 as iio

# # Assuming you have defined 'bbox.site_id' and 'images.imagepath'
# gif_file = f"../../output/figures/swot/map/pixc/gif/pixc_swe_{bbox.site_id}_v01.gif"

# # Create GIF animation from images using imageio.v3
# with iio.get_writer(gif_file, mode='I', duration=20, loop=1, fps=1) as writer:
# 	for image_path in images.imagepath:
# 		image = iio.imread(image_path)  # Read image using imageio.v3
# 		writer.append_data(image)

# import imageio
# # Create GIF animation from images
# gif_file = "../../output/figures/swot/map/pixc/gif/pixc_swe_" + bbox.site_id + "_v01.gif"

# with imageio.get_writer(gif_file, duration=20, loop=1, fps=1) as writer:  #  mode='I',  fps
# 	for image in images.imagepath:
