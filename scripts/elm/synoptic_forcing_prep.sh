...
# Change dir to the COMPASS_synoptic_sims dir
cd elm/elmdata/COMPASS_synoptic_sims




# # Outputs from Ben's make_COMPASS_ELM_forcing.py
...
synoptic = \
    (pd.read_csv("data/raw/transect_coords/compass_synoptic.csv")
      .assign(grid_points=lambda x: x["site_id"].astype(str) + "_" + x["zone_id"].astype(str))
...
# Loop through 21 gridpoints (7 sites x 3 zones each)
...
#%%  Define distance and height from stream
...
#%% NOTE: E.Fluet: I don't have Kaizad's data beyond this point.
